...
import { validateEmail, validatePassword, validateForm, hasFormErrors } from '../../utils/validation';
import { FormErrors, TranslationObject, ScreenType } from '../../types/index';
...
export function Login({ onNavigate, t }: LoginProps) {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      setErrors(prev => ({
        ...prev,
        submit: error instanceof Error ? error.message : '로그인에 실패했습니다'
      }));
    } finally {
      setIsLoading(false);
    }
    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <div className="flex-1 -mt-6 px-4">
        <Card className="max-w-[375px] mx-auto p-6 shadow-lg">
          {/* Error Message */}
          {errors.submit && (
            <div className="flex items-start gap-3 p-3 mb-4 bg-destructive/10 rounded-lg">
          ...
          {/* Forgot Password */}
          <button
            className="text-primary mt-2 w-full text-right hover:text-primary/80 transition-colors disabled:opacity-50"
            disabled={isLoading}
          >
          ...
          <Button
            onClick={handleLogin}
            disabled={isLoading}
            className="w-full mt-6 bg-primary hover:bg-[#2563EB] text-primary-foreground h-12 rounded-xl disabled:opacity-50"
          >
            {isLoading ? '로그인 중...' : t.auth.loginButton}
